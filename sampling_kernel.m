...
% Convolution with sampling kernel
kernel_width = 0.02; % Kernel width (adjust based on signal characteristics)
kernel = exp(-((t - mean(t)).^2) / (2 * kernel_width^2)); % Gaussian kernel
kernel = kernel / sum(kernel); % Normalize kernel
x_filtered = conv(x, kernel, 'same'); % Convolve signal with kernel

% Update x with filtered signal
x = x_filtered / max(x_filtered); % Normalize filtered signal

% ============================ Section 2: Fourier Coefficient Calculation =========================
% Deine parameters for Fourier analysis
K = 7; % Number of harmonics to consider
