...
% Load ECG signal
signal = load('C:\Users\97253\OneDrive - Ben Gurion University of the Negev\Desktop\IF-TEM\ecg_Data\GDN0010\GDN0010_2_Valsalva.mat'); 
x = signal.tfm_ecg2(6.91e5:6.93e5); % Extract signal segment
x = x';
...
numPermutations = 100; % Adjust for computational efficiency
errorMin = inf;
bestSignal = [];
bestParams = [];

% Permute over all possible parameter variations
for permIdx = 1:numPermutations
    tk_perm = tk + 0.01 * (rand(1, K) - 0.5); % Small random variation
    rk_perm = rk + 0.01 * (rand(1, K) - 0.5); % Small random variation
    ck_perm = ck + 0.01 * (rand(1, K) - 0.5); % Small random variation
    
    % Reconstruct the signal
    t = linspace(0, T, N); % Time vector
    signal2 = zeros(size(t));
    for k = 1:K
        signal2 = signal2 + time_eval(ck_perm, rk_perm, tk_perm, t, k);
    end
...
figure;
plot(t, x / max(x), 'k', 'LineWidth', 4); hold on;
plot(t, bestSignal, '--r', 'LineWidth', 4);
