...
from misc.alg_tools_1d import iqml_recon, iqml_recon_ri, periodic_sinc, distance,iqml_recon_ri2

import misc.matrix_operations as mat_op
...
class VPW_FRI_2D_Radial(object):
    ...
    def evaluate_time_domain_pulse(self, x, y, k):
        ...
        if (self.rk.ndim == 1):
            return self.ck[k] / (np.pi * np.pi) * self.rk[k] / (np.sqrt(np.power(
                np.power(self.rk[k], 2) + np.power(x_grid - self.tk[k, 0], 2) + np.power(y_grid - self.tk[k, 1], 2),
                3)))
        else:
            return self.ck[k] / (np.pi * np.pi) * self.rk[k, 0] / (np.sqrt(np.power(
                np.power(self.rk[k, 0], 2) + np.power(x_grid - self.tk[k, 0], 2) + np.power(y_grid - self.tk[k, 1], 2),
                3)))
    ...
    def evaluate_Fourier_domain_pulse(self, f_u, f_v, k):
        ...
        if (self.rk.ndim == 1):
            ...
            return self.ck[k] * np.exp(-1j * self.tk[k, 0] * u_grid - 1j * self.tk[k, 1] * v_grid) * np.exp(
                -self.rk[k] * np.sqrt(np.power(u_grid, 2) + np.power(v_grid, 2))) / (self.T1 * self.T2)
        else:
            ...
            return self.ck[k] * np.exp(-1j * self.tk[k, 0] * u_grid - 1j * self.tk[k, 1] * v_grid) * np.exp(
                -np.sqrt(np.power(u_grid * self.rk[k, 0], 2) + np.power(v_grid * self.rk[k, 1], 2))) / (
                               self.T1 * self.T2)
...
class VPW_FRI_2D(object):
    ...
    def evaluate_time_domain_pulse(self, x, y, k):
        ...
        y_grid, x_grid = np.meshgrid(y, x)
        if (self.rk.ndim == 1):
            return 2 * self.ck[k] / np.pi * self.rk[k] / (
                        np.power(x_grid - self.tk[k, 0], 2) + self.rk[k] * self.rk[k]) * self.rk[k] / (
                               np.power(y_grid - self.tk[k, 1], 2) + self.rk[k] * self.rk[k])
        else:
            return self.ck[k] / np.pi * self.rk[k, 0] / (
                        np.power(x_grid - self.tk[k, 0], 2) + self.rk[k, 0] * self.rk[k, 0]) * self.rk[k, 1] / (
                               np.power(y_grid - self.tk[k, 1], 2) + self.rk[k, 1] * self.rk[k, 1])
    ...
    def evaluate_Fourier_domain_pulse(self, f_u, f_v, k):
        ...
        v_grid, u_grid, = np.meshgrid(f_v, f_u)
        if (self.rk.ndim == 1):
            return self.ck[k] * np.exp(-1j * self.tk[k, 0] * u_grid - self.rk[k] * np.abs(u_grid)) * np.exp(
                -1j * self.tk[k, 1] * v_grid - self.rk[k] * np.abs(v_grid)) / (self.T1 * self.T2)
        else:
            return self.ck[k] * np.exp(-1j * self.tk[k, 0] * u_grid - self.rk[k, 0] * np.abs(u_grid)) * np.exp(
                -1j * self.tk[k, 1] * v_grid - self.rk[k, 1] * np.abs(v_grid)) / (self.T1 * self.T2)
...
class FRI_estimator(object):
    ...
    def estimate_parameters_iqml2(self, signal, coefficients, G=None, stop_cri='max_iter', max_ini=10, noise_level=0):

        #IQML
        signal = signal.reshape([np.shape(signal)[0],])
        if G is None:
            G = self.construct_G_iqml(signal)
        y = np.hstack((np.real(signal), np.imag(signal[1:])))
        print(np.shape(y))
        print(np.shape(G))
        xhat_recon, min_error, c_opt, ini = iqml_recon_ri2(G, y, self.K, noise_level, max_ini, stop_cri)
    ...
    def savitzky_golay(y, window_size, order, deriv=0, rate=1):
        ...
        try:
            ...
            TypeError("window_size size must be a positive odd number")
